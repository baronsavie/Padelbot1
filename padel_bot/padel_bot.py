#!/usr/bin/env python3
"""Padel Bot v8.6 – Dual-Account | 3 Schiebe-Modi | Court-2-Präferenz

FIXES v8.6:
  • Früh-Methode: 2-3s Dauerbeschuss nur auf 07:00 (Court 2→1), dann sofort
    Fallback auf frühesten freien Slot – kein 300s-Leerlauf mehr
  • berechne_freie_slots: Exakte 7-Tage-Grenze (datetime statt Tages-Ebene)
    → Direkte Taktik bucht erst wenn der Slot wirklich buchbar ist
  • Session-Refresh 60s vor 07:00 / vor Direktstart (aus v8.5)
  • Session-Check alle 50 Versuche im Dauerbeschuss (aus v8.5)
  • Session-Check vor Stornierung + Neubuchung im Schiebe-Loop (aus v8.5)
  • Stündlicher Session-Check bei langen Wartezeiten (aus v8.5)
...
MENÜ-STRUKTUR:
  ...
  Ebene 3 → Schiebe-Modi:
    ...
    🎯 Direkte Taktik – User gibt "buchbar ab" per Text ein (z.B. "17:30")
                        → Bot wartet bis heute 17:30, bucht dann 25.04 17:30,
                          und schiebt schrittweise zur Wunschzeit
...
# Wie viele Versuche NUR auf 07:00 bevor Fallback (ca. 2-3 Sekunden)
...
def berechne_freie_slots(k: str, datum_de: str, dauer_min: int,
    ...
    FIX v8.6: Exakte 7-Tage-Grenze (datetime, nicht Tages-Ebene).
    Beispiel: Jetzt = 18.04 14:00 → grenze = 25.04 14:00
    → 25.04 13:30 ist buchbar, 25.04 16:30 noch NICHT.
    """
    ...
    # FIX v8.6: Exakte Grenze – kein Auf-Tages-Ebene-Runden mehr
    grenze = jetzt + timedelta(days=7)

    freie = []
    for court in [2, 1]:   # Court 2 bevorzugt
        ...
        while t <= letzter_start:
            ...
            if t_bis > schluss:
                t += timedelta(minutes=30)
                continue
            # Exakter Vergleich: Slot-Startzeit am Buchungstag vs. Grenze
...
def _aggressiv_buchen_07(k: str, datum_de: str, datum_api: str, dauer_min: int) -> bool:
    ...
    while time.time() < deadline:
        ...
        # ── Phase A: Erste ~2s exklusiv auf 07:00 ──────────────────
        ...
        # ── Phase B: Fallback – frühesten freien Slot nehmen ───────
        # Einmalig melden wenn wir von Phase A in Phase B wechseln
        if versuch == FRUEH_EXKLUSIV_VERSUCHE:
        ...
        # Court 2 kommt zuerst (bereits in berechne_freie_slots so sortiert)
        if freie:
            fallback = freie[0]
...
def _aggressiv_buchen_ab(k: str, datum_de: str, datum_api: str, dauer_min: int,
    ...
    Court 2 bevorzugt. Session alle 50 Versuche prüfen.
    """
    schluss  = datetime.strptime(ANLAGE_SCHLUSS, "%H:%M")
    deadline = time.time() + AGGRESSIVE_TIMEOUT
    versuch  = 0
    ...
    while time.time() < deadline:
        ...
        if versuch > 0 and versuch % 50 == 0:
            if not ist_eingeloggt(k):
                log.warning(f"[{k}] Dauerbeschuss (ab): Session abgelaufen")
                senden(f"🔑 [{k}] Session im Dauerbeschuss abgelaufen – logge neu ein...")
                if not einloggen(k):
                    senden(f"❌ [{k}] Neu-Login fehlgeschlagen!")
                    return False
...
def _schiebe_intern(k: str):
    ...
    if modus == "frueh":
        ...
        if jetzt < start_07:
            ...
            # 60s vor 07:00 aufwachen und einloggen
            ...
            # 07:00 schon vorbei → trotzdem frisch einloggen
    ...
    # ── 🎯 DIREKT-MODUS ───────────────────────────────────────────
        ...
        """
        FIX v8.6 – Direkte Taktik korrekt:
        buchbar_ab = "17:30" bedeutet: heute um 17:30 Uhr wird der 25.04 17:30-Slot buchbar.
        → Bot wartet bis heute 17:30, bucht dann 25.04 um 17:30, schiebt zur Wunschzeit.
        berechne_freie_slots gibt 17:30 am 25.04 erst nach 17:30 heute als frei zurück,
        weil grenze = jetzt + 7 Tage (exakt).
        """
        if not buchbar_ab:
        ...
        if jetzt < buchbar_dt:
            ...
            # 60s vorher aufwachen und einloggen
            ...
            # Startzeit schon vorbei → frisch einloggen
    ...
    # Warte bis Slot-Ende - SCHIEBE_MINUTEN_VOR → storniere → buche nächsten
    # Nächster Start = aktuelles Ende - 30 Min (vorwärts)
    # ══════════════════════════════════════════
    stuck_zaehler          = {}
    ...
    while aktiv():
        ...
        # Schiebe-Moment: HEUTE zur Uhrzeit (Ende - SCHIEBE_MINUTEN_VOR)
        # (Buchung ist für datum_obj, aber Schieben passiert HEUTE im 7-Tage-Fenster)
        jetzt = jetzt_lokal()
        ...
        if sek > 0:
            ...
            while aktiv():
                ...
                if not schlafe(chunk):
                    return
                # Stündlicher Session-Check bei langen Wartezeiten
        ...
        # Session vor Stornierung sicherstellen
        if not stelle_session_sicher(k):
        ...
        # Session vor Neubuchung sicherstellen
        if not stelle_session_sicher(k):
...
if __name__ == "__main__":
    log.info("=" * 60)
    log.info("🎾 Padel Bot v8.6 – Dual-Account | 3 Schiebe-Modi | Court-2-Prio")
    ...
    log.info(f"   Früh exklusiv   : {FRUEH_EXKLUSIV_VERSUCHE} × {AGGRESSIVE_INTERVAL}s "
             f"≈ {FRUEH_EXKLUSIV_VERSUCHE * AGGRESSIVE_INTERVAL:.1f}s nur auf 07:00")
    log.info(f"   Schiebe         : {SCHIEBE_MINUTEN_VOR} Min vor Slot-Ende")
    log.info(f"   Aggressive      : {AGGRESSIVE_TIMEOUT}s @ {AGGRESSIVE_INTERVAL}s")
    log.info(f"   Session-Refresh : 60s vor Aktion | alle 50 Versuche | stündlich")
    log.info("=" * 60)
    ...
    for k in ACCOUNTS:
        if not einloggen(k):
            ...
            log.info(f"   [{k}] PERSON_ID: {az_get(k, 'person_id') or 'NICHT GEFUNDEN'}")
    ...
    senden("🎾 <b>Padel Bot v8.6 gestartet!</b>\n\n"
           "Dual-Account  |  3 Schiebe-Modi  |  Court 2 bevorzugt\n"
           "🌅 Früh: ~2s exklusiv auf 07:00 → Fallback\n"
           "🎯 Direkt: bucht exakt zur buchbaren Zeit")
    zeige_account_auswahl()
    ...
    try:
        ...
        senden("⏹️ Padel Bot v8.6 wurde beendet.")
